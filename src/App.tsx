...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubAttendance = onSnapshot(query(collection(db, 'attendance'), orderBy('tanggal', 'desc')), (snap) => {
      setAttendance(snap.docs.map(d => d.data() as Attendance));
    }, (error) => handleFirestoreError(error, OperationType.GET, 'attendance'));

    const unsubTeacherAttendance = onSnapshot(query(collection(db, 'teacherAttendance'), orderBy('tanggal', 'desc')), (snap) => {
    ...
    const unsubTeachingSchedules = onSnapshot(collection(db, 'teachingSchedules'), (snap) => {
